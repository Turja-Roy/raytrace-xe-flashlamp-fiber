...
def get_fiber_position_hybrid(origins, dirs, lens1, lens2, z_l2, f2_mm,
                               fiber_rad, acceptance_half_rad, medium='air',
                               pressure_atm=1.0, temp_k=293.15, humidity_fraction=0.01,
                               n_samples=20, coupling_threshold=0.05, length_threshold=65.0):
    """
    Hybrid fiber positioning: Use paraxial approximation when it works well,
    fall back to optimized search for problematic systems.
    
    Logic:
    1. Try paraxial approximation: z_fiber = z_l2 + f2_mm
    2. Calculate coupling at paraxial position
    3. If coupling < threshold AND z_fiber > length_threshold:
       - System appears broken, use find_optimal_fiber_position() to search
    4. Else:
       - Keep paraxial position (fast, backward compatible with well-behaved systems)
    
    Parameters:
    - origins, dirs: Nx3 arrays of ray origins and directions
    - lens1, lens2: PlanoConvex objects
    - z_l2: z-position of lens2
    - f2_mm: paraxial focal length of lens2 (mm)
    - fiber_rad: fiber core radius
    - acceptance_half_rad: half-acceptance angle in radians
    - medium: propagation medium
    - pressure_atm: pressure in atmospheres
    - temp_k: temperature in Kelvin
    - humidity_fraction: water vapor fraction
    - n_samples: number of z positions to test if optimization triggered
    - coupling_threshold: trigger optimization if coupling below this (default 0.05)
    - length_threshold: trigger optimization if z_fiber above this (default 65mm)
    
    Returns:
    - z_fiber: fiber position (mm)
    - coupling: coupling efficiency at fiber position
    """
    from scripts.raytrace_helpers_vectorized import trace_system_vectorized
    
    # Try paraxial approximation first
    z_fiber_paraxial = z_l2 + f2_mm
    
    accepted, transmission = trace_system_vectorized(
        origins, dirs, lens1, lens2, z_fiber_paraxial,
        fiber_rad, acceptance_half_rad, medium,
        pressure_atm, temp_k, humidity_fraction
    )
    
    avg_transmission = np.mean(transmission[accepted]) if np.any(accepted) else 0.0
    coupling_paraxial = (np.count_nonzero(accepted) / len(origins)) * avg_transmission
    
    # Check if system is problematic (low coupling AND long system)
    if coupling_paraxial < coupling_threshold and z_fiber_paraxial > length_threshold:
        # System appears broken - search for better fiber position
        z_search_start = z_l2 + lens2.center_thickness_mm + 0.5
        z_search_end = z_l2 + max(f2_mm * 2.0, 50.0)
        
        z_fiber_opt, coupling_opt = find_optimal_fiber_position(
        ...
        # Paraxial works well, use it (fast path)
        return z_fiber_paraxial, coupling_paraxial
