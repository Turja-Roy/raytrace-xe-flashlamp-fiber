...
def optimize_single_lens(lenses, name, n_rays=1000, alpha=0.7, medium='air', orientation_mode='both', seed=None):
    """
    Optimize single-lens configuration.
    ...
    results = []
    orientations = [
        (False, 'ScfF'),  # curved-first (S = source, c = curved, f = flat, F = fiber)
        (True, 'SffF')    # flat-first (S = source, f = flat, f = flat, F = fiber)
    ]
    ...
    for flipped, orientation_name in orientations:
        ...
        if best_result is None:
            ...
            logger.error(f"  Tried {len(initial_guesses)} starting points, all returned penalty values")
            continue
        
        result = best_result
        logger.info(f"Best optimization result: objective={result.fun:.6f}, x={result.x}")
    ...
    if orientation_mode == 'both':
        ...
        result_to_return = results[0] if results else None
